...
void Elevator::initUI(int pos, int whichEle, MainWindow* win)
{
    //显示楼层数码管
    ...
    //报警键
    alert = new QPushButton(win);
    alert->setGeometry(QRect(pos + 40, 980, 51, 51));
    alert->setStyleSheet(
                "QPushButton{image: url(:/pic/alert.png); background: white;}");
    connect(alert, &QPushButton::clicked, this, [=](){eleAlert(win,whichEle);});
}
...
void ElevatorControl::updateEle()
{
    for(int i = 0; i < ELEVATOR_NUM; i++)
    {
        ...
        if(!ele[i]->msgQueue->empty())
            ele[i]->eleThread = QtConcurrent::run(ele[i], &Elevator::run, waitQueue);
        else
            ele[i]->eleStatus->setStyleSheet("QLabel{image: url(:/pic/stop.png);}");
    }

    //不使用多线程的电梯运行代码(每两秒所有电梯刷新一次)
